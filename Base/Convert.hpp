...
i32 hexCharToDecimal(utf8Char& hexChar)
{
    utf8Char c = hexChar;
    if(c <= 97 && c >= 65) c = hexChar + 32;
...
i32 hexToDecimal(utf8Char* hex,i64 lenght)
{
    i32 sumOfHex = 0;
    for(i64 i = 0,b = lenght-1;i < lenght; ++i,--b)
    {
        sumOfHex += (hexCharToDecimal(hex[i]) * powerOf16(b));
    }
    return sumOfHex;
}
